...
class TInvestFloatingBondService:
    def __init__(self, client):
        self.client = client

    def get_floating_bond_positions(
        self,
        account_id: str,
        from_date: date,
        to_date: date,
        ...
        for position in portfolio.positions:
            if (getattr(position, "instrument_type", "") or "").lower() != "bond":
                continue
            bond_position, floating_coupon_flag = self._build_bond_position(position, from_date, to_date)
            if floating_coupon_flag:
                positions.append(bond_position)
        return positions

    def _build_bond_position(self, position: Any, from_date: date, to_date: date) -> tuple[BondPosition, bool]:
        ...
        return (
            ...
            bool(getattr(bond, "floating_coupon_flag", False)),
        )
